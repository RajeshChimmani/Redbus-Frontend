  ...
  const makePayment = async (token) => {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        alert("Booking successful! Redirecting to profile...");
        history.push("/my-profile");
      }
    } catch (err) {
      console.error("Error while adding booking to booking collection!", err);
      alert("Booking failed. Please try again. Error: " + (err.response?.data?.error || err.message));
    }
